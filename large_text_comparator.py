...
class CompareThread(QThread):
    ...
    finished_signal = pyqtSignal(dict, dict, dict, dict, dict, float)  # 增加 intersection
    ...
    def run(self):
        ...
        self.progress_signal.emit(80, "Calculating unique & intersection items...")
        unique_in_a = {val: map_a[val] for val in sorted(set_a - set_b)}
        unique_in_b = {val: map_b[val] for val in sorted(set_b - set_a)}
        intersection = {val: (map_a[val]) for val in sorted(set_a & set_b)}

        self.progress_signal.emit(100, "Completed")
        self.finished_signal.emit(dup_a, dup_b, unique_in_a, unique_in_b, intersection, start)
...
class LargeTextComparatorApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Large text comparator v0.5.3 by:xwk")
    ...
    def create_result_tabs(self):
        ...
        self.tab_widget.addTab(self.intersection_tab, "List A intersection List B")
    ...
    def on_compare_finished(self, dup_a, dup_b, unique_in_a, unique_in_b, intersection, start):
        self.update_result_tabs(dup_a, dup_b, unique_in_a, unique_in_b, intersection)
    ...
    def update_result_tabs(self, dup_a, dup_b, unique_in_a, unique_in_b, intersection):
        ...
        self.update_intersection_table(self.intersection_tab, intersection, "Original Text")
        ...
        self.tab_widget.setTabText(4, f"List A intersection List B({format_with_thousands_separator(len(intersection))})")
    ...
    def update_intersection_table(self, tab, data, header_orig):
        if tab.layout():
            QWidget().setLayout(tab.layout())
        layout = QVBoxLayout(tab)
        table = CopyableTableWidget()
